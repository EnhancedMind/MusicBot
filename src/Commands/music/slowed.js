...
module.exports = new Command({
	...
	async run(message, args, client) {
        ...
        queue.unpipe(message.guild.id);
        queue.player(message.guild.id, { inherit: true, localPath: path, speed: args[0] ? Number(args[0]) : 0.85 });

		response.edit(`${success} Downloaded track to be slowed... Playing now!`);
	}
});
